...
impl Map {
    pub fn new(a_node: Rc<Node>) -> Option<Map> {
        let a_map_node_option = a_node.search_child_by_name("map");
        let map_node: Rc<Node>;
        match a_map_node_option {
            None => {eprintln!("Not possible to find child named 'map' in the current Node!"); return None}
            Some(a_map_node) => {map_node = a_map_node}
        }
        // We get the COLORS
        let colors_node_option: Option<Rc<Node>> = map_node.search_child_by_name("colors");
        ...
        // We get the SYMBOLS
        let symbols_node_option: Option<Rc<Node>> = map_node.search_child_by_name("symbols");
